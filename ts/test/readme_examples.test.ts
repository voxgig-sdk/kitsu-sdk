...
//     (\`new KitsuSDK(...)\` / \`KitsuSDK.test(...)\` become
//     \`KitsuSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new KitsuSDK(...)\` and any
// \`KitsuSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KitsuSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KitsuSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

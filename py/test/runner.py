# Kitsu SDK test runner

from __future__ import annotations
import os
import json

from kitsu_sdk.utility.voxgig_struct import voxgig_struct as vs


class KitsuTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KitsuTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KitsuTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KitsuTestRunner.getenv("KITSU_TEST_LIVE")
        override = KitsuTestRunner.getenv("KITSU_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KitsuTestRunner.getenv(key)
        ...
        explain = KitsuTestRunner.getenv("KITSU_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["KITSU_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if KitsuTestRunner._test_control is not None:
            return KitsuTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KitsuTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KitsuTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KitsuTestRunner.load_test_control()
...
def load_env_local():
    KitsuTestRunner.load_env_local()


def env_override(m):
    return KitsuTestRunner.env_override(m)


def entity_data(v):
    return KitsuTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KitsuTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KitsuTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KitsuTestRunner.load_test_control()


def live_client_options():
    return KitsuTestRunner.live_client_options()


def live_delay_ms():
    return KitsuTestRunner.live_delay_ms()

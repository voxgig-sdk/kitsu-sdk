    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set KITSU_TEST_ANIME_ENTID JSON to run live")
        client = setup["client"]
...
def _anime_basic_setup(extra):
    ...
    client = KitsuSDK.test(options, extra)
    ...
    idmap_resolved = helpers.to_map(
        env.get("KITSU_TEST_ANIME_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("KITSU_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = KitsuSDK(helpers.to_map(merged_opts))

    _live = env.get("KITSU_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("KITSU_TEST_EXPLAIN") == "TRUE",
        "live": _live,

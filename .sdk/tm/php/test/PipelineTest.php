...
{
    private static function utility(): KitsuUtility
    {
        return new KitsuUtility();
    }
    ...
    private static function ctx(array $over = []): KitsuContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new KitsuContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new KitsuOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof KitsuError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new KitsuOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = KitsuMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new KitsuOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = KitsuMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new KitsuOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = KitsuMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KitsuSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = KitsuMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new KitsuResponse([]),
            'result' => new KitsuResult([]),
        ]);
        [, $err] = KitsuMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new KitsuSpec(['step' => 's']),
            'response' => new KitsuResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new KitsuResult([]),
        ]);
        [, $err] = KitsuMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new KitsuSpec(['step' => 's']),
            'response' => new KitsuResponse(self::resp(200, ['v' => 1])),
            'result' => new KitsuResult([]),
        ]);
        [, $err] = KitsuMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new KitsuSpec(['step' => 's']),
            'response' => new KitsuResponse(self::resp(200, ['v' => 2])),
            'result' => new KitsuResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        KitsuMakeResponse::call($ctx);
    ...
    {
        $preset = new KitsuResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new KitsuSpec([]),
            'response' => new KitsuResponse([]),
            'result' => new KitsuResult([]),
        ...
        [$got, $err] = KitsuMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new KitsuResult([])]);
        [, $err] = KitsuMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new KitsuSpec([]), 'result' => null]);
        [, $err] = KitsuMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KitsuSpec(['step' => 's']),
            'result' => new KitsuResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new KitsuOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KitsuMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KitsuSpec(['step' => 's']),
            'result' => new KitsuResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new KitsuOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KitsuMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KitsuResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new KitsuSpec([]),
            'result' => new KitsuResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = KitsuMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new KitsuError('boom', 'boom');
        $utility->fetcher = function (KitsuContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KitsuSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = KitsuMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (KitsuContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = KitsuMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(KitsuResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (KitsuContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KitsuSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = KitsuMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new KitsuResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new KitsuSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = KitsuMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = KitsuMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = KitsuMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new KitsuResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, KitsuDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new KitsuResult(['ok' => false])]);
        $this->expectException(KitsuError::class);
        KitsuDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new KitsuResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', KitsuMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new KitsuBaseFeature();
        $b = new KitsuBaseFeature();
        KitsuFeatureAdd::call($ctx, $a);
        KitsuFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): KitsuBaseFeature
    {
        $f = new KitsuBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): KitsuContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new KitsuSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new KitsuResponse(['headers' => null]),
            'result' => new KitsuResult([]),
        ]);
        KitsuResultHeaders::call($ctx);

...
class FtCtrl extends KitsuControl
{
...
class FtClient
{
    ...
    private KitsuUtility $utility;

    public function __construct(KitsuUtility $utility, array $options)
    {
    ...
    public function get_utility(): KitsuUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KitsuContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KitsuContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KitsuConfig::make_feature($name);
    ...
    private static function buildUrl(KitsuSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KitsuError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KitsuResponse($fetched);
        }
    ...
    private function fail(KitsuContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KitsuContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

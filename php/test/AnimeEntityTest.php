...
class AnimeEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set KITSU_TEST_ANIME_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function anime_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("KITSU_TEST_ANIME_ENTID");
    ...
    if ($env["KITSU_TEST_LIVE"] === "TRUE") {
        ...
        $client = new KitsuSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["KITSU_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,

...
//    only - a typed KitsuContext there would make every ctx
...
final class KitsuOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new KitsuContext($clean, null);
    ...
    public static function err_from_map($m): ?KitsuError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new KitsuError(is_string($code) ? $code : '', $msg);
    }

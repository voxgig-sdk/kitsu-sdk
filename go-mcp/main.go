// Command Kitsu-mcp is an MCP server exposing the Kitsu
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Kitsu_list  — list records of an entity
//	Kitsu_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: KITSU_APIKEY carries the API key and
	// KITSU_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KITSU_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KITSU_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKitsuSDK(opts)

...
[MCP](https://modelcontextprotocol.io) server exposing the Kitsu SDK as
two agent tools — `kitsu_list` and `kitsu_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kitsu-mcp)
make build

# 2. Provide credentials via the environment
export KITSU_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user kitsu \
  -- /absolute/path/to/kitsu-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./kitsu-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// kitsu_list: first page of records
{ "entity": "anime" }
{ "entity": "anime", "query": { } }

// kitsu_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kitsu-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user kitsu \
     -- "$PWD"/dist/*/kitsu-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `kitsu_list` and `kitsu_load` tools now appear
   in new sessions. Ask the agent to *"list anime using kitsu"*
   and it calls `kitsu_list` with `{"entity":"anime"}`.
...
```sh
export KITSU_APIKEY=sk_live_xxx            # API key
export KITSU_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `kitsu_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `kitsu_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KITSU_APIKEY` | API key sent with every request. |
| `KITSU_BASE` | Optional override of the API base URL. |
...
```sh
./kitsu-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"kitsu_load","arguments":{"entity":"anime","query":{"id":1}}}}'
```

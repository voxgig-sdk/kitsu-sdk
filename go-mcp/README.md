...
MCP server exposing the Kitsu SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `kitsu_list` | `entity`, optional `query` map | First page of records as JSON |
| `kitsu_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `kitsu_list` and
`kitsu_load` tools become available in new sessions.

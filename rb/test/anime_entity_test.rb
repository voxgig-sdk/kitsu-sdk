...
class AnimeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set KITSU_TEST_ANIME_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def anime_basic_setup(extra)
  ...
  entid_env_raw = ENV["KITSU_TEST_ANIME_ENTID"]
  ...
  if env["KITSU_TEST_LIVE"] == "TRUE"
    ...
    client = KitsuSDK.new(Helpers.to_map(merged_opts))
  end

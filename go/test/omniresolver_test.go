...
import (
	...
	omni "github.com/voxgig-sdk/kitsu-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KitsuError); is {
				out := map[string]any{
					"name":    "KitsuError",
...
func omniCtx(arg any, client *sdk.KitsuSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KitsuSDK); is {
			client = live

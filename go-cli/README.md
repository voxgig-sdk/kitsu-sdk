...
boru-driven command-line client **and** interactive REPL for the Kitsu
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kitsu-cli)
make build
...
./kitsu-cli load 1 anime            # {id:1} shorthand
./kitsu-cli load '{id:1}' anime       # explicit match map

# 5. Override the API base URL for a single call
KITSU_BASE=https://api.example.com ./kitsu-cli load 1 anime
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kitsu-cli
   ```
...
```sh
./kitsu-cli load 1 anime          # scalar shorthand for {id:1}
./kitsu-cli load '{id:1}' anime     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `kitsu>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/kitsu-cli`. |
...
(`./kitsu-cli <expr>`) and interactively (the REPL), and expressions compose the
